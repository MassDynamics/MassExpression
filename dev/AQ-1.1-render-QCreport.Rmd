...
design_1 <- design[-1, ]
design_2 <- design
design_2 <- rbind(design_2, design[4,])
design_2$SampleName[4] <- "Extra_1"

intensities_1 <- intensities[, -10]
intensities_2 <- intensities
intensities_2$Extra <- intensities[, 5]

design_3 <- design_2 %>% filter(Condition %in% "sample2")
design_3 <- design_3[-1,]

listIntensityExperiments <- runGenericDiscovery(experimentDesign = design_3, 
                                                proteinIntensities = intensities_2, 
                                                normalisationMethod = normalisation_method,
                                                species = species, 
                                                labellingMethod = labellingMethod)

# 1. Error when there are more samples in the design than in the protein intensity columns 
# 
...
design <- design %>% dplyr::rename(SampleName = IntensityColumn) %>%
  dplyr::mutate(Replicate_1 = Replicate) %>%
...
parameters <- data.frame(V1 = c("Species", "UseNormalisationMethod", "LabellingMethod"),
                         V2 = c("Other", "None", "LFQ"))

write_delim(design, "../data-private/experiment_design.tsv", delim = "\t")
write_delim(intensities, "../data-private/protein_intensities.tsv", delim = "\t")
write_delim(parameters, "../data-private/parameters.tsv", delim = "\t", col_names = FALSE)
```
...
write_delim(design, "../data-private/maxquant/experiment_design.tsv", delim = "\t")
write_delim(intensities, "../data-private/maxquant/protein_intensities.tsv", delim = "\t")
write_delim(parameters, "../data-private/maxquant/parameters.tsv", delim = "\t", col_names = FALSE)
