...
construct_summarized_experiment <- function(experiment.design, protein.intensities){
  ...
  rownames(summarized.experiment) <- rowData(summarized.experiment)$ProteinId
...
# workflow step)
run_generic_discovery <- function(protein.intensities, experiment.design){
...
# count the imputations in each condition and write to wide table
...
create_comparison_experiments <- function(complete.protein.summarized.experiment,
                                         comparison){
...
```{r}
get_list_comparison_experiments <- function(complete.protein.summarized.experiment){
  
  comparisons = 
    get_comparison_strings(rowData(complete.protein.summarized.experiment))
  
  
  comparison.experiments <- list()
  
  for (comparison in comparisons){
    comparison.experiments[[comparison]] = create_comparison_experiments(
      complete.protein.summarized.experiment, comparison
    )
  }
...
comparison.experiments <- 
  get_list_comparison_experiments(complete.protein.summarized.experiment)


save(protein.summarized.experiment, 
     complete.protein.summarized.experiment, 
     prot.int,
     comparison.experiments,
     file = "DiscoveryQuant.RData")


load("DiscoveryQuant.RData")
```

```{r}
library(EnrichmentAnalysisStepR)
gmt_folder = "/home/joseph/github_repos/enrichment-step/gene_sets/10090/"
...
```{r anna-test, eval=FALSE}
fp_exp <- design_fragpipe
fp_proteins <- assay_fragpipe

run_generic_discovery(protein.intensities = fp_proteins, experiment.design = fp_exp)
```
...
## Wrap QC Report compilation

Getting the relevant code for running qc, putting it in output folder
and cleaning up. This will be quite md specific

```{r}


# get qc report from package
  file.copy(from=system.file("rmd","QC_Report.Rmd", package = "MassExpression"),
            to=output_folder,
...
# render it in the output folder
