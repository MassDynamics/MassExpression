...
1. *Experiment Design and Summary of Results*: overview of the experiment design used in the analysis.
2. *Experiment Health*: this includes dimensionality reduction plots using all and differentially expressed (DE) proteins; distribution of the coeffiecient of variations by conditions and smaple correlations using all and DE proteins.
...
design <- design %>% tidyr::unite(SampleNameInPlots, Condition, Replicate, 
                                                      sep="_", remove=FALSE) %>%
  dplyr::select(-Replicate, SampleName, Condition, SampleNameInPlots, everything())
  
if(params$format == "pdf"){
  knitr::kable(as_tibble(design), row.names = FALSE) 
  #%>%
...
The table below reports the number of proteins considered for each pairwise comparison analysis (proteins with more than 50% missing values across samples are removed) and the number of DE proteins detected in each comparison. The overall total number of proteins included in the exeriment is `r nrow(rowData(CompleteIntensityExperiment))`.
...
summary_de <- n_proteins_by_cond_long %>% 
  group_by(Comparison) %>%
  summarise(`Total Number of Proteins` = sum(!is.na(AdjPval)),
...
<details>
  ...
  <span>
    ...
    <ul>
      ...
      <li>Including only differentially expressed (DE) proteins. The DE proteins are defined as those proteins with an adjusted p-value < 0.05, where the p-value is the one of the limma ANOVA test which tests for differences using all categories of the condition of interest jointly. At least 5 DE proteins are required to produce the second PCA plot.</li>
    </ul>
  ...
  <span>
   ...
   <p>The sections reported here show:</p>
    <ul>
      ...
      <li>3. The distribution of the imputed and not imputed intensities<li>
    </ul>
...
<details>
  ...
  <span>
   ...
   <p>For a healthy experiment we expect:</p>
    <ul>
      <li>1. The RLE boxplots to have a similar median - centered around zero - across all samples </li>
      <li>2. The RLE boxplots to have a similar width of the boxes across samples </li>
    </ul>
...
## B. Imputed vs not imputed {.tabset .tabset-fade .tabset-pills}
...
```{r compare-imputed-condition}
plot_imputed_vs_not(CompleteIntensityExperiment = CompleteIntensityExperiment, byCondition = TRUE)
```
