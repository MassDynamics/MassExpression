...
```{r pca-imputed}
## PCA
p=pca_plot_experiment(CompleteIntensityExperiment, format = params$format)
p[[1]]
```
...
```{r pca-imputed-de}
p=pca_plot_experiment(CompleteIntensityExperiment, format = params$format, onlyDEProteins = TRUE)
p[[1]]
```
...
## C. Sample correlations using DE proteins


```{r corr-samples}
plot_samples_correlation_matrix(CompleteIntensity)
```


```{r corr-samples-de}
plot_samples_correlation_matrix(CompleteIntensity, onlyDEProteins = TRUE)
```
...
# 4. Normalisation and Imputation {.tabset .tabset-fade .tabset-pills}

<details>
  <summary>Distributions of raw and normalised intensities (when requested)</summary>
  <span>
   ...
   <p>The three sections reported here show:</p>
    <ul>
      <li>The boxplots of the log2 intensities before any normalisation </li>
      <li>The boxplots of raw relative log expression (RLE) values and after normalisation, when requested. The RLE values for a protein are obtained by centering intensities to the protein medians, where the median is computed using only available intensities, i.e. non zero values. </li>
...
## Raw Intensities boxplots

Boxplots of log2 raw intensities. 
...
p_raw <- plot_measurement_boxplot(IntensityExperiment, log = TRUE)
