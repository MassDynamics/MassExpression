...
<details>
  ...
  <span>
    <p>It is useful to inspect the distribution of the Relative Log Expression (RLE) values to identify replicates with largely unusual distributions. The RLE values for a protein are obtained by substracting the median of each protein across all the samples where that protein is available, i.e. non zero values. The medians of the RLE boxplots should be roughly centered on zero. </p>
   <p>For a healthy experiment we expect:</p>
    <ul>
      <li>The RLE boxplots to have a similar median, centered around zero, across all replicates </li>
    </ul>
     <p>If some replicates show large devations of the boxplots medians from zero, it can be symptomatic of problems in the experiment.<p>
  </span>
</details> 
